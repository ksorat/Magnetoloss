...
#xR[-1] = 1
xR = [15,15,1]
gs=gridspec.GridSpec(Np,2+1, width_ratios=xR)
...
for i in range(2):
	for j in range(Np):
		...
		if (np.mod(i,2) == 0):
			Sc = ax.scatter(s,z,c=Q,cmap=cm,vmin=-0.25,vmax=0.25)
		else:
			Mask = (alph>=80) & (alph<=100)
			Sc = ax.scatter(s[Mask],z[Mask],c=Q[Mask],cmap=cm,vmin=-0.25,vmax=0.25)
